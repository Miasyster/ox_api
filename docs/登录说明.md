...
2. **运行程序**
   ...
   **方法A：使用启动脚本（推荐）**
   ```bash
   # 在 demo 目录下运行 start.bat
   cd demo
   start.bat
   ```
   启动脚本会自动切换到 bin 目录，确保 Tradelog.prop 等配置文件能被正确找到。
   ...
   **注意**：程序启动时会自动尝试切换到 bin 目录，以确保 Tradelog.prop 配置文件中的相对路径（如 `./LOG/tradeapi.log`）能够正确解析。
...
3. **工作目录**: 程序会自动尝试切换到 `bin` 目录，以确保 `Tradelog.prop` 等配置文件能被正确找到。如果程序无法自动切换，请手动切换到 `bin` 目录运行，或使用 `start.bat` 启动脚本
4. **网络连接**: 需要稳定的网络连接到国信证券交易服务器
5. **交易时间**: 登录功能可以在非交易时间使用，但某些查询功能可能受限
6. **配置文件**: 确保配置文件中的账户信息准确无误
